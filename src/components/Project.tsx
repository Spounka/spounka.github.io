  ...
  return (
    <motion.div className={"flex gap-1.5 z-10 cursor-pointer"}>
      ...
      <div
        ...
        <motion.div
          className={"flex content-start justify-start gap-x-5 h-0 -translate-y-1 overflow-hidden"}
          animate={controls}
          transition={{ duration: 0.3, ease: "easeInOut" }}>
          {link && (
            <a className="flex gap-1 align-middle bg-white rounded-md p-1" href={link} target={"_blank"}>
              <img src={link_svg} alt="link logo" className="h-5" />
            </a>
          )}
          {github && (
            <a className="flex gap-1 align-middle bg-white rounded-md p-1" href={github} target={"_blank"}>
